...
void Testing::update()
{
	if (this->m_collided)
	{
		...
		{
			this->m_collided = false;
			s2d::Sprite::deleteSpriteByName("name");
			thisSprite->transform.position = s2d::Vector2(-500, 500);
			up->transform.position = s2d::Vector2(-1000, 0);
			down->transform.position = s2d::Vector2(-1000, 0);
		}
	}
	...
	std::string score = "fps: " + std::to_string(s2d::Time::fps);
