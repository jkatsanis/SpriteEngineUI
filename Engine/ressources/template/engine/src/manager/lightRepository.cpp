...
void s2d::LightRepository::updateLightSource(s2d::Sprite* sprite, bool call_by_update, const s2d::Camera* camera)
{
	...
	if (sprite->transform.position_changed || call_by_update || true)
	{
		...
		s2d::Vector2 new_pos = s2d::Vector2((sprite->transform.getPosition().x + 960) + 960 * zoom, a);
...
{
	if (!LightRepository::s_m_light_shader.loadFromFile("engine/ressources/shaders/circulaer_gradient.frag", sf::Shader::Fragment))
	{
...
{
	float zoom = s2d::GameObject::camera.getZoom();
